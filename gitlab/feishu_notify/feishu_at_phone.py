...
if __name__ == '__main__':
    ...
    # today=datetime.date(2022, 7, 25).weekday()
    # week_num =datetime.date(2022, 7, 25).isocalendar()[1]
    print("today",today)
    print("week_num",week_num)
    web_hook = 'https://open.feishu.cn/open-apis/bot/v2/hook/' + args.key

    duty = [['robot@test.com','xxxxxxx']]

    if week_num % len(duty)>=3:
        weekend_duty_index=week_num % len(duty)-3
    else:
        weekend_duty_index=week_num % len(duty)+2

    duty_list =duty
    duty_list.append(duty[weekend_duty_index])
    duty_list.append(duty[weekend_duty_index])
    duty_list.append(duty[0])
    user1 = duty_list[today]
    user2 = duty_list[today+1]
    print (user1,user2)
