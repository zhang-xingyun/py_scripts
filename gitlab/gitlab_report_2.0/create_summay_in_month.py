...
class Report():
    def __init__(self,
        ...
        self.project_list = self.parse_xml(self.get_xml_file(self.include_tag))
        #print('project_list' + str(self.project_list))

    def get_commit_info_in_month(self):
        self.get_commit_info_in_several_month(self.include_month, self.project_list)
    ...
    def get_commit_info_in_several_month(self, month_num, proj_list):
        ...
        #print('\n---------------TOTAL RESULT:---------------\n')
        print('month_data result is: ' + str(month_data))
        #return month_data
        self.generate_excel(total_result, 'report/commit_by_several_month', 'between')
...
def main():
    ...
    parser.add_argument('-b', '--include_branch',
                        ...
                        type=str)
    parser.add_argument('-t', '--include_tag',
                        metavar=str(), required=False,
                        help="input include tag: manifest的xml文件，来指示需要从哪些repo里获取数据，文件名不需要添加.xml",
                        type=str)
    parser.add_argument('-m', '--include_month',
                        metavar=str(), required=False,
                        help="input include month: 需要生成几个月跨度的数据",
                        type=str)

    args = parser.parse_args()

    report = Report(args.include_repo, args.include_branch, args.include_tag, args.include_month)
    report.get_commit_info_in_month()
