...
class Report():
    def __init__(self, 
        ...
        if include_group is not None and len(include_group) > 0:
            self.include_group = list(map(lambda x:int(x), include_group.split(',')))
        else:
            self.include_group = None
        if include_repo is not None and len(include_repo) > 0:
            self.include_repo = list(map(lambda x:int(x), include_repo.split(',')))
        else:
           self.include_repo = list()
        if exclude_group is not None and len(exclude_group) > 0:
            self.exclude_group = list(map(lambda x:int(x), exclude_group.split(',')))
        else:
            self.exclude_group = None
        if exclude_repo is not None and len(exclude_repo) > 0:
            self.exclude_repo = list(map(lambda x:int(x), exclude_repo.split(',')))
        else:
        ...
        self.raw_groups = self.read_pkl(self.raw_data.raw_data_path + 'groups_attributes.pkl')
    ...
    def generate_members_excel(self):
        ...
        for project_id in self.query_projects:
            ...
            #if belong_to == 'auto' and self.raw_projects[project_id]['name_with_namespace'].find('平台与技术部') == 0:
            #    continue
...
def main():
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.dedent('''\
                     ...
                     '''))
    parser.add_argument('-ig', '--include_group',
                        metavar=str(), required=False,
                        help="input include group",
                        type=str)
    parser.add_argument('-ir', '--include_repo',
                        metavar=str(), required=False,
                        help="input include repo",
                        type=str)
    parser.add_argument('-eg', '--exclude_group',
                        metavar=str(), required=False,
                        help="input exclude group",
                        type=str)
    parser.add_argument('-er', '--exclude_repo',
                        metavar=str(), required=False,
                        help="input exclude repo",
                        type=str)
    parser.add_argument('-bb', '--branch_block',
                        type=bool,
                        default=False)

    args = parser.parse_args()

    report = Report(args.include_group, args.include_repo, args.exclude_group, args.exclude_repo, args.branch_block)
