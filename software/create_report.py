...
class Report():
    def __init__(self):
        ...
        if include_group is not None and len(include_group) > 0:
            self.include_group = list(
                map(lambda x: int(x), include_group.split(',')))
        else:
            self.include_group = None
        if include_repo is not None and len(include_repo) > 0:
            self.include_repo = list(
                map(lambda x: int(x), include_repo.split(',')))
        else:
            self.include_repo = list()
        if exclude_group is not None and len(exclude_group) > 0:
            self.exclude_group = list(
                map(lambda x: int(x), exclude_group.split(',')))
        else:
            self.exclude_group = None
        if exclude_repo is not None and len(exclude_repo) > 0:
            self.exclude_repo = list(
                map(lambda x: int(x), exclude_repo.split(',')))
        else:
            self.exclude_repo = None
        self.branch_block = branch_block
        self.branch_block_day = 365
        #
    ...
    def get_query_projects(self):
        ...
        if self.include_repo:
            project_list.extend(self.include_repo)
        if self.exclude_repo:
    ...
    def generate_members_excel(self):
        ...
        total_manage = 0
        total_miss = 0
        project_user_permission_file_path = \
            self.raw_data_path + '/project_user_permission.pkl'
        print(project_user_permission_file_path)
        if not os.path.exists(project_user_permission_file_path):
            return
        project_user_permission = self.read_pkl(
            project_user_permission_file_path)
        for project_id in self.query_projects:
            ...
            if self.raw_projects[project_id]['description']:
                description = self.raw_projects[project_id][
                    'description'].split('\r\n')
            ...
            if not self.raw_projects[project_id]['archived']:
                total += 1
            p_ret = False
            for permission, user_list in project_user_permission[
                    project_id].items():
                for username in user_list:
                    if username in []:
                        continue
                    tmp_dict = dict()
                    tmp_dict['name_with_namespace'] \
                        = self.raw_projects[project_id]['name_with_namespace']
                    tmp_dict['project_path'] = self.raw_projects[project_id][
                        'web_url']
                    tmp_dict['username'] = username
                    tmp_dict['permission'] = permission
                    tmp_dict['freeze'] = self.raw_projects[project_id][
                        'archived']
        ...
        print(summary)
        self.generate_excel([summary], 'report/member_summary',
                            'member_summary')
    ...
    def generate_project_excel(self):
        ...
        for project_id in self.query_projects:
            ...
            project_last_activity_at = project['last_activity_at'].split('T')[
                0]
            project_group = 'http://gitlab.test.com/' + project['namespace'][
                'full_path']
            ...
            tmp_project['inactive_day'] = int((self.current_time - time.mktime(
                time.strptime(project_last_activity_at, '%Y-%m-%d'))) / (
                24 * 60 * 60))
            tmp_project['type'] = 'normal'
            tmp_project['freeze'] = False
            if project['archived']:
                tmp_project['freeze'] = 'True'
            branch_file_path = self.raw_data_repo_path + str(
                project_id) + '/branches_attributes.pkl'
            ...
            if project['description']:
                ...
                if len(description) >= 6:
                    ...
                    if description[4].find('分支管控') == 0:
                        tmp_project['分支管控'] = description[4].replace(
                            '分支管控:', '')
                    if description[5].find('审批人') == 0:
                        tmp_project['审批人'] = description[5].replace(
                            '审批人:', '')
            # print(description)
    ...
    def generate_project_approver_excel(self):
        output_project = list()
        error_list = list()
        ...
        for project_id, project_data in projects_attributes.items():
            # if not project_id == 7484:
            #     continue
            repo_tmp = dict()

            try:
                repo = self.gl.projects.get(project_id, statistics=True)
            except Exception as e:
                error_info = 'error project exception ' + \
                             repo.attributes['web_url'] + str(e)
                print(error_info)
                error_list.append(error_info)
                continue
            repo_attr = repo.attributes
            inactive_day = int(
                (current_time - time.mktime(
                    time.strptime(repo_attr['last_activity_at'].split('T')[0],
                                  '%Y-%m-%d'))) / (24 * 60 * 60))
            repo_tmp['repo_id'] = project_id
            repo_tmp['inactive_day'] = inactive_day
            repo_tmp['web_url'] = repo_attr['web_url']
            repo_tmp['visibility'] = repo_attr['visibility']
            repo_tmp['storage_size'] = repo_attr['statistics'][
                'storage_size']
            # get branches
            try:
                ...
                error_info = 'error branch exception ' + repo_attr[
                    'web_url'] + str(e)
                print(error_info)
                error_list.append(error_info)
                continue
            # get tags
            try:
                tags = repo.tags.list(all=True, retry_transient_errors=True)
            except Exception as e:
                error_info = 'error tag exception ' + repo_attr[
                    'web_url'] + str(e)
                print(error_info)
                error_list.append(error_info)
                continue
            repo_tmp['branch_num'] = len(branches)
            repo_tmp['tag_num'] = len(tags)
            active_member = list()
            guest = list()
            reporter = list()
            developer = list()
            ...
            mem = repo.members_all.list(all=True)
            if mem:
                print(len(mem))
                for m in mem:
                    if m.attributes['state'] == 'active':
                        active_member.append(m.attributes['username'])
                        if m.attributes['access_level'] == 40:
                            maintainer.append(m.attributes['username'])
                        if m.attributes['access_level'] == 30:
                            developer.append(m.attributes['username'])
                        if m.attributes['access_level'] == 20:
                            reporter.append(m.attributes['username'])
                        if m.attributes['access_level'] == 10:
                            guest.append(m.attributes['username'])
            repo_tmp['members'] = len(active_member)
            repo_tmp['maintainer'] = str(maintainer)
            repo_tmp['developer'] = str(developer)
            repo_tmp['reporter'] = str(reporter)
            repo_tmp['guest'] = str(guest)
            approver = models.RepoApprover.objects.filter(
                repo_id__repo_id=project_id)
            if approver:
                if approver[0].approver.strip() == 'robot':
                    repo_tmp['approver'] = '仓库管理员'
                else:
                    repo_tmp['approver'] = approver[0].approver
            else:
                repo_tmp['approver'] = '仓库管理员'
            month_list = ['2022-04', '2022-05', '2022-06']
            commits = models.Commit.objects.filter(
                repo_id__repo_id=project_id,
                submitter_month__in=month_list)
            commit_per_week = int(len(commits) / (len(month_list) * 4))
            repo_tmp['commits_per_week'] = commit_per_week
            print(repo_tmp)
            output_project.append(repo_tmp)
        if len(output_project) > 0:
            self.generate_excel(
                output_project,
                'report/project_approver',
                'project_approver'
            )
    ...
    def generate_branch_excel(self):
        ...
        for project_id in self.query_projects:
            ...
            if self.raw_projects[project_id]['description']:
                ...
                if len(description) >= 6:
                    if description[4].find('分支管控') == 0:
                        release_branch = description[4].replace('分支管控:',
                                                                '').split('|')
            #
            ...
            commits = dict()
            commits_file_path = self.raw_data_path + '/repo/' + str(
                project_id) + '/commits.pkl'
            ...
            for branch_name, branch in branches_attributes.items():
                ...
                tmp_dict['project_namespace'] = self.raw_projects[project_id][
                    'name_with_namespace']
                tmp_dict['project_path'] = self.raw_projects[project_id][
                    'web_url']
                tmp_dict['project_group'] = 'http://gitlab.test.com/' + \
                                            self.raw_projects[project_id][
                                                'namespace']['full_path']
                ...
                if branch_name in commits.keys():
                    for i in commits[branch['name']]:
                        if i['submitter_data'] > tmp_dict[
                                'branch_last_commit']:
                            tmp_dict['branch_last_commit'] = i[
                                'submitter_data']
                if len(tmp_dict['branch_last_commit']) == 0:
                    if branch_name in commits.keys():
                        tmp_dict['branch_last_commit'] \
                            = commits[branch['name']][-1]['submitter_data']
                    else:
                        if len(commits) > 0:
                            tmp_dict['branch_last_commit'] \
                                = commits[list(commits.keys())[0]][-1][
                                'submitter_data']
                        else:
                            tmp_dict['branch_last_commit'] = branch_create
                tmp_dict['inactive_day'] = 0
                if not tmp_dict['branch_last_commit'] == 'more_unknown':
                ...
                if self.branch_block and not tmp_dict['freeze']:
                    ...
                    if tmp_dict['inactive_day'] > 365 and block_branch_ret:
                        print('block branch',
                              self.raw_projects[project_id]['web_url'],
                              branch_name)
                        try:
                            ctl_project = \
                            ...
                            ctl_branch = \
                                ctl_project.branches.get(
                                    branch_name, retry_transient_errors=True)
                            ctl_branch.unprotect(retry_transient_errors=True)
                            ctl_project.protectedbranches.create(
                                {'name': branch_name, 'merge_access_level': 0,
                                 'push_access_level': 0},
                                retry_transient_errors=True)
                        except Exception as e:
                            print('block branch error',
                                  self.raw_projects[project_id]['web_url'],
                                  branch_name, str(e))
                # sys.exit(1)
                tmp_dict['标记'] = None
                if branch_name in release_branch:
                    tmp_dict['标记'] = '主干/发布'
                    if branch_name.lower().find(
                            'rel') == 0 or branch_name.lower().find(
                            'sprint') == 0:
                        tmp_dict['标记'] = '主干/发布'
                ...
                if len(description) >= 6:
                    if description[4].find('分支管控') == 0:
                        release_branch = description[4].replace('分支管控:',
                                                                '').split('|')
            #
            conuts_attributes_file_path = self.raw_data_path + '/repo/' + str(
                project_id) + '/counts.pkl'
    ...
    def generate_commit_excel(self):
        for project_id in self.query_projects:
            output = dict()
            #
            commits_attributes_file_path = self.raw_data_path + '/repo/' + str(
                project_id) + '/commits.pkl'
    ...
    def generate_single_commit_excel(self):
        ...
        for project_id in self.query_projects:
            #
            single_commits_attributes_file_path = self.raw_data_path + 'repo/'\
                + str(project_id) + '/single_commits.pkl'
            # print(single_commits_attributes_file_path)
            if not os.path.exists(single_commits_attributes_file_path):
                continue
            single_commits_attributes = self.read_pkl(
                single_commits_attributes_file_path)
            for single, commit in single_commits_attributes.items():
                ...
                tmp_dict['jira_type'] = ','.join(tmp_dict['jira_type'])
                if tmp_dict['jira_id']:
                    tmp_dict['jira_type'] = self.find_jira_type(
                        tmp_dict['jira_id'])
                year = tmp_dict['date'].split('-')[0]
                if year not in output.keys():
                    output[year] = list()
        ...
        for year, data in output.items():
            if len(data) > 0:
                self.generate_excel(data, 'report/single_commit_' + year,
                                    'single_commit')
    ...
    def generate_mrs_excel(self):
        ...
        for project_id in self.query_projects:
            ...
            mrs_attributes_file_path = self.raw_data_path + '/repo/' + str(
                project_id) + '/mrs_attributes.pkl'
            ...
            for mr in mrs_attributes:
                ...
                for pipeline in mr['pipeline']:
                    tmp_pipeline = dict()
                    if isinstance(pipeline, type(tmp_pipeline)):
                        print(pipeline)
                        tmp_pipeline.update(pipeline)
                    else:
                        tmp_pipeline['id'] = pipeline.id
                        tmp_pipeline['sha'] = pipeline.sha
                        tmp_pipeline['ref'] = pipeline.ref
                        tmp_pipeline['status'] = pipeline.status
                ...
                tmp_output_participant_list = list(
                    set(tmp_output_participant_list))
                ...
                for discussion in mr['discussion']:
                    ...
                    if discussion['system']:
                        if discussion['body'] == 'merged' or discussion[
                                'body'] == 'closed':
                            tmp_dict['closed_or_merged'] = \
                                discussion['updated_at'].split('T')[0]
                        continue
                    ...
                    tmp_discussion['created_at'] = \
                        discussion['created_at'].split('T')[0]
                    tmp_discussion['updated_at'] = \
                        discussion['updated_at'].split('T')[0]
                    tmp_discussion['project_path'] = \
                        self.raw_projects[project_id]['web_url']
        ...
        if len(output_pipeline) > 0:
            self.generate_excel(output_pipeline, 'report/mr_pipeline',
                                'mr_pipeline')
        if len(output_discussion) > 0:
            self.generate_excel(output_discussion, 'report/mr_discussion',
                                'mr_discussion')
        if len(output_participant) > 0:
            self.generate_excel(output_participant, 'report/mr_participant',
                                'mr_participant')
    ...
    def generate_issues_excel(self):
        ...
        for project_id in self.query_projects:
            #
            issues_attributes_file_path = self.raw_data_path + '/repo/' + str(
                project_id) + '/issues_attributes.pkl'
