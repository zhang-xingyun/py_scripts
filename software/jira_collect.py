...
class GetData():
    def __init__(self):
        ...
        self.jira_conn = JIRA('https://jira.test.com:8443',
                              auth=('', ''))
        self.jira_raw_folder = '/opt/gitlab_report/jira_data/'
        self.issue_folder = '/opt/gitlab_report/jira_data/issue/'
    ...
    def get_projects(self):
        ...
        jira_project_path = os.path.join(self.jira_raw_folder,
                                         'jira_project.pkl')
        self.save_pkl(jira_project_path, self.projects_attributes)

    def get_issues(self):
        for project_key in self.projects_attributes.keys():
            issue_raw = dict()
            sql = 'project=' + project_key + ' AND Created >= "2024-1-1"'
            issues_in_proj = self.jira_conn.search_issues(
                sql, maxResults=-1)
            for issue in issues_in_proj:
                issue_raw[issue.key] = issue.raw
            issue_path = os.path.join(self.issue_folder,
                                      '%s.pkl' % project_key)
