...
def gen_ph_cr_times_by_gitlab_path(gitlab_path: str, from_time: str):
    repos = models.Repo.objects.filter(web_url__icontains=gitlab_path)
    repo_ids = [item.repo_id for item in repos]
    cr_list = models.CodeReview.objects.filter(repo_id__in=repo_ids, source='PH'
                                               , start_time__gt=from_time)
    data = list()
    for cr in cr_list:
        ...
        if len(comments) == 0:
            ...
            for comment in comments:
                ...
                data.append(tmp)
    print(data)
    generate_excel(data, 'report/ph_cr_times-' + 'ss', 'ph_cr_times')
...
def run():
    ...
    gen_ph_cr_times_by_gitlab_path("https://gitlab.test.com/ptd/ss", '2023-07-01')
