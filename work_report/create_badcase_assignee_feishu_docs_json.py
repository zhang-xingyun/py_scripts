...
from hdflow.work_report.jira_badcase_report import (
    JiraBadcaseReport,
    badcase_objtype_assignee_statistics,
    filter_change_status_issue,
)

__all__ = ["jira_badcase_assignee_report_to_feishu_doc"]
...
def _init_feishu_table():
    column_infos = [
        ...
        {"name": "方向", "width": 100},
        {"name": "Assignee", "width": 200},
        {"name": "待研发定位总数量", "width": 150},
        {"name": "从待研发定位变更为研发解决中", "width": 150},
        {"name": "研发解决中总数量", "width": 150},
        {"name": "从研发解决中变更为验证中", "width": 150},
        {"name": "验证中总数量", "width": 150},
    ]
    table = Table(
    ...
    if badcase_list is not None:
        for badcase in badcase_list:
            if badcase.status in [
                ...
                verified_badcases.append(badcase)
        ...
        working_to_verified_issues = filter_change_status_issue(
            issues=verified_badcases,
            from_status=[
                IssueStatus.Solving_RD_cn.value,
                IssueStatus.Working.value,
            ],
    ...
    table.insert_row(
        tablerow=TableRow(
            tableCells=[
                ...
                _create_person_tablecell(assignee=assignee),
                _create_textrun_tablecell(
                ...
                _create_textrun_tablecell(
                    text=str(len(working_to_verified_issues)),
                    link=jira_jql_link.format(
                        url=jira_url_prefix,
                        issue_keys=", ".join(
                            [
                                issue.issue_key
                                for issue in working_to_verified_issues
                            ]
                        ),
                    ),
                ),
                _create_textrun_tablecell(
                    text=str(len(verified_badcases)),
                    link=jira_jql_link.format(
                        url=jira_url_prefix,
                        issue_keys=", ".join(
                            [issue.issue_key for issue in verified_badcases]
                        ),
                    ),
                ),
            ]
...
def _create_content_blocks(
    ...
    for project_name, jira_badcases in jira_badcase_set.items():
        ...
        results = badcase_objtype_assignee_statistics(
            badcase_list=badcase_list
        )
        results = sorted(results.items(), key=lambda x: x[0])
        project_row_index = 0
        for objtype, assignees in results:
            for assignee, badcase_list in assignees.items():
            ...
            table.append_mergedcell(
                MergedCell(
                    rowStartIndex=table.rowSize - len(assignees),
                    rowEndIndex=table.rowSize,
                    columnStartIndex=1,
                    columnEndIndex=2,
                )
            )
            project_row_index += len(assignees)
...
def jira_badcase_assignee_report_to_feishu_doc(
